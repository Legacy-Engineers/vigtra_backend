...
def optimize_queryset(
    queryset, select_related_fields=None, prefetch_related_fields=None
):
    """
...
def get_database_stats():
    ...
    stats = {
        ...
        "queries": connection.queries[-10:]
        if connection.queries
        else [],  # Last 10 queries
    }
...
def analyze_model_queries(model_class):
    ...
    if fk_fields:
        suggestions.append(
            f"Consider using select_related({', '.join(fk_fields)}) for {model_name}"
        )

    # Check for reverse foreign key relationships
    reverse_fk_fields = [
        f.get_accessor_name() for f in model_class._meta.related_objects
    ]
    if reverse_fk_fields:
        suggestions.append(
            f"Consider using prefetch_related({', '.join(reverse_fk_fields)}) for {model_name}"
        )
