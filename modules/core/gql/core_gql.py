...
class CoreMutation(graphene.Mutation):
    ...
    def mutate(cls, root, info, **kwargs):
        ...
        """
        try:
            mutation: dict = cls.perform_mutation(root, info, **kwargs)
            ...
            change_log = ChangeLog(
                ...
                user=info.context.user if getattr(info.context.user, "is_authenticated", False) else None,
                request_header=info.context.META,
    ...
    @classmethod
    def perform_mutation(cls, root, info, **kwargs) -> dict:
        """
        Subclasses must implement this method to perform the actual mutation logic.
        """
        raise NotImplementedError("Subclasses must implement the perform_mutation method")