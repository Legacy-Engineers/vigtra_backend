...
class ContributionPlan(
    ...
    code = models.CharField(
        max_length=20, unique=True, help_text=_("Unique code for the contribution plan")
    )

    name = models.CharField(
        max_length=100, help_text=_("Name of the contribution plan")
    )

    description = models.TextField(
        blank=True, null=True, help_text=_("Detailed description of the plan")
    )
    ...
    grace_period_days = models.PositiveIntegerField(
        default=30, help_text=_("Grace period in days for late payments")
    )

    # Eligibility Criteria
    min_age = models.PositiveIntegerField(
        blank=True, null=True, help_text=_("Minimum age for eligibility")
    )

    max_age = models.PositiveIntegerField(
        blank=True, null=True, help_text=_("Maximum age for eligibility")
    )
    ...
    is_mandatory = models.BooleanField(
        default=False, help_text=_("Whether participation in this plan is mandatory")
    )

    allows_partial_payment = models.BooleanField(
        default=False, help_text=_("Whether partial payments are allowed")
    )

    auto_renew = models.BooleanField(
        default=True, help_text=_("Whether contributions auto-renew")
    )
    ...
    created_date = models.DateTimeField(
        auto_now_add=True, help_text=_("Date plan was created")
    )

    last_modified = models.DateTimeField(
        auto_now=True, help_text=_("Date plan was last modified")
    )

    notes = models.TextField(
        blank=True, null=True, help_text=_("Administrative notes about the plan")
    )
        ...
        if self.calculation_type in [
            ...
            if not self.percentage_rate:
                raise ValidationError(
                    {
                        "percentage_rate": _(
                            "Percentage rate is required for percentage-based calculations"
                        )
                    }
                )
        ...
        if self.calculation_type == ContributionCalculationType.CUSTOM_FORMULA:
            if not self.custom_formula:
                raise ValidationError(
                    {
                        "custom_formula": _(
                            "Custom formula is required for custom calculations"
                        )
                    }
                )
    ...
    def calculate_contribution(self, **kwargs):
        ...
        if self.calculation_type == ContributionCalculationType.FIXED_AMOUNT:
            ...
            tiers = (
                self.tiered_rates.filter(min_income__lte=income)
                .order_by("-min_income")
                .first()
            )
            ...
            try:
                ...
                amount = Decimal(
                    str(eval(self.custom_formula, {"__builtins__": {}}, namespace))
                )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["contribution_frequency"], name="idx_contrib_plan_frequency"
            ),
            models.Index(
                fields=["validity_from", "validity_to"],
                name="idx_contrib_plan_validity",
            ),
        ]
        constraints = [
            ...
            models.CheckConstraint(
                ...
                | Q(max_contribution__isnull=True)
                | Q(min_contribution__isnull=True),
