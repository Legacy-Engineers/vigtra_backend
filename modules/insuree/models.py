...
class FamilyType(BaseCodeModel):
    ...
    type = models.CharField(
        ...
        max_length=50,
        help_text=_("Family type description")
    )
    is_active = models.BooleanField(
        default=True,
        help_text=_("Whether this family type is active")
    )
...
class Family(core_models.VersionedModel, core_models.ExtendableModel, LifecycleModel):
    ...
    poverty = models.BooleanField(
        db_column="Poverty",
        default=False,
        help_text=_("Whether family is in poverty")
    )
    ...
    created_date = models.DateTimeField(
        auto_now_add=True,
        help_text=_("Date family was created")
    )
    
    last_modified = models.DateTimeField(
        auto_now=True,
        help_text=_("Date family was last modified")
    )
    
    notes = models.TextField(
        blank=True,
        null=True,
        help_text=_("Additional notes about the family")
    )
        ...
        if self.head_insuree:
            ...
            if not active_membership:
                ...
                if not any_membership:
                    raise ValidationError(_("Head insuree must have a membership in this family"))
    ...
    def add_member(self, insuree, is_head=False, relationship=None, start_date=None):
        ...
        if existing_membership:
            raise ValidationError(_("Insuree is already an active member of this family"))
    ...
    def _assign_new_head(self):
        ...
        if adult_members:
            ...
            new_head_membership = min(adult_members, 
                                    key=lambda m: m.insuree.dob or datetime.date.min)
        elif potential_heads:
            # If no adults, pick oldest member
            new_head_membership = min(potential_heads, 
                                    key=lambda m: m.insuree.dob or datetime.date.min)
        else:
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['validity_from', 'validity_to'], name='idx_family_validity'),
        ]
        constraints = [
            models.CheckConstraint(
                check=Q(confirmation_no__isnull=True) | Q(confirmation_no__length__gte=3),
...
class Profession(models.Model):
    ...
    is_active = models.BooleanField(
        default=True,
        help_text=_("Whether profession is active")
    )
    sort_order = models.PositiveSmallIntegerField(
        default=0,
        help_text=_("Sort order for display")
...
class Education(models.Model):
    ...
    code = models.CharField(
        ...
        blank=True,
        null=True,
        help_text=_("Education code")
    )
    level = models.PositiveSmallIntegerField(
        default=0,
        help_text=_("Education level rank")
    )
    is_active = models.BooleanField(
        default=True,
        help_text=_("Whether education level is active")
    )
...
class IdentificationType(models.Model):
    ...
    is_active = models.BooleanField(
        default=True,
        help_text=_("Whether identification type is active")
    )
    requires_validation = models.BooleanField(
        default=False,
        help_text=_("Whether this ID type requires validation")
    )
...
class Relation(models.Model):
    ...
    relation = models.CharField(
        db_column="Relation",
        max_length=50,
        help_text=_("Relationship description")
    )
    code = models.CharField(
        max_length=10,
        unique=True,
    ...
    is_active = models.BooleanField(
        default=True,
        help_text=_("Whether relationship is active")
    )
    sort_order = models.PositiveSmallIntegerField(
        default=0,
        help_text=_("Sort order for display")
...
class FamilyMembership(core_models.VersionedModel, LifecycleModel):
    ...
    is_head = models.BooleanField(
        default=False,
        help_text=_("Whether this member is head of family")
    )
    ...
    membership_start_date = models.DateField(
        ...
        help_text=_("Date membership started")
    )
    
    membership_end_date = models.DateField(
        blank=True,
        null=True,
        help_text=_("Date membership ended")
    )
    
    notes = models.TextField(
        blank=True,
        null=True,
        help_text=_("Notes about this membership")
    )
    
    created_date = models.DateTimeField(
        auto_now_add=True,
        help_text=_("Date membership was created")
    )
    
    last_modified = models.DateTimeField(
        auto_now=True,
        help_text=_("Date membership was last modified")
    )
    ...
    def clean(self):
        ...
        if self.is_head and self.family:
            ...
            if other_heads.exists():
                raise ValidationError({'is_head': _("Family can only have one active head")})
        ...
        if self.status != FamilyMembershipStatus.ACTIVE and not self.membership_end_date:
    ...
    def set_end_date_for_inactive(self):
        ...
        if (self.has_changed('status') and 
            self.status != FamilyMembershipStatus.ACTIVE and 
            not self.membership_end_date):
            self.membership_end_date = datetime.date.today()
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['family', 'status'], name='idx_membership_family_status'),
            models.Index(fields=['insuree', 'status'], name='idx_membership_insuree_status'),
            models.Index(fields=['validity_from', 'validity_to'], name='idx_membership_validity'),
        ]
        constraints = [
...
class Insuree(core_models.VersionedModel, core_models.ExtendableModel, LifecycleModel):
    ...
    # Remove direct family relationship - now handled through FamilyMembership
    # family = models.ForeignKey(
    #     Family,
    #     on_delete=models.CASCADE,
    #     blank=True,
    ...
    #     help_text=_("Family this insuree belongs to")
    # )
    ...
    offline = models.BooleanField(
        db_column="isOffline",
        default=False,
        help_text=_("Whether created offline")
    )
    ...
    created_date = models.DateTimeField(
        auto_now_add=True,
        help_text=_("Date insuree was created")
    )
    
    last_modified = models.DateTimeField(
        auto_now=True,
        help_text=_("Date insuree was last modified")
    )
    ...
    def clean(self):
        ...
        if self.identification and self.identification.requires_validation:
            if not self.identification_number:
                raise ValidationError({
                    'identification_number': _("Identification number is required for this type")
                })
            
            if self.identification.validation_regex:
                import re
                if not re.match(self.identification.validation_regex, self.identification_number):
                    raise ValidationError({
                        'identification_number': _("Invalid identification number format")
                    })
    ...
    def current_family(self):
        ...
        except (FamilyMembership.DoesNotExist, FamilyMembership.MultipleObjectsReturned):
            return None
    ...
    def current_family_membership(self):
        ...
        try:
            return self.family_memberships.get(
                status=FamilyMembershipStatus.ACTIVE
            )
        except (FamilyMembership.DoesNotExist, FamilyMembership.MultipleObjectsReturned):
            return None
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['validity_from', 'validity_to'], name='idx_insuree_validity'),
            models.Index(fields=['identification_number'], name='idx_insuree_id_number'),
        ]
        constraints = [
            models.UniqueConstraint(
                fields=['chf_id'],
    ...
    card_issued_date = models.DateField(
        blank=True,
        null=True,
        help_text=_("Date card was issued")
    )
